...
난 이게 싫음!!! HashSet만의 본래 add메소드를 멀쩡하게 호출하고 싶어! 자식이 재정의한 메소드 말고!      </br></br>
...
메소드 안에 또다른 메소드 호출이 없게 할거야. 괜히 그랬다가는 그 호출되는 메소드를 자식이 재정의 해버리면 또 의도치 않게 자신의 메소드가 아닌 자식의 재정의 메소드가 호출되어 문제가 발생할테니까.   </br></br>     
